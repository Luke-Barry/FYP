...
async def run_client(host: str, port: int) -> None:
    os.makedirs("/app/qlogs", exist_ok=True)  # Ensure qlog directory exists
    ...
    async with connect(host, port, configuration=configuration, create_protocol=MyQuicConnectionProtocol) as protocol:
        ...
        tasks = [asyncio.create_task(send_from_queue(protocol, queue, queue_name)) for queue_name, queue in queues.items()]
