...
def get_orderbook():
    try:
        ...
        response = requests.get("http://quic-client:6060/api/orderbook")
...
def get_user_orders(user_id):
    try:
        # Forward request to QUIC client
        response = requests.get(f"http://quic-client:6060/api/orders/{user_id}")
        if response.status_code == 200:
            return jsonify(response.json()), 200
        else:
...
def place_order():
    ...
    try:
        ...
        response = requests.post(
            'http://quic-client:6060/api/order',
            json=data,
